...
class UVsim:
    def __init__(self, simulator):
        ...
        self.branding_label = tk.Label(self.root, text="UVsim", font=(
            "Helvetica", 24, "bold"), bg=self.primary_color, fg=get_contrasting_color(self.primary_color))
        ...
        self.new_tab_button = tk.Button(self.control_frame, text="New Main Tab", command=self.create_new_main_tab,
                                        width=15, bg=self.off_color, fg=get_contrasting_color(self.off_color))
        self.new_tab_button.grid(row=0, column=0, padx=5)

        self.load_button = tk.Button(self.control_frame, text="Load Program", command=self.load_program,
                                     width=15, bg=self.off_color, fg=get_contrasting_color(self.off_color))
        self.load_button.grid(row=0, column=1, padx=5)

        self.run_button = tk.Button(self.control_frame, text="Run", command=self.run_program,
                                    width=15, bg=self.off_color, fg=get_contrasting_color(self.off_color))
        self.run_button.grid(row=0, column=2, padx=5)

        self.reset_button = tk.Button(self.control_frame, text="Reset", command=self.reset_simulator,
                                      width=15, bg=self.off_color, fg=get_contrasting_color(self.off_color))
        self.reset_button.grid(row=0, column=3, padx=5)

        self.rerun_button = tk.Button(self.control_frame, text="Re-run", command=self.rerun_program,
                                      width=15, bg=self.off_color, fg=get_contrasting_color(self.off_color))
        ...
        self.save_as_button = tk.Button(self.control_frame, text="Save As", command=self.save_program_as,
                                        width=15, bg=self.off_color, fg=get_contrasting_color(self.off_color))
    ...
    def create_menus(self):
        ...
        file_menu.add_command(label="New Main Tab",
                              command=self.create_new_main_tab)
        ...
        edit_menu.add_command(
            label="Cut", command=lambda: self.get_current_text_area().event_generate("<<Cut>>"))
        edit_menu.add_command(
            label="Copy", command=lambda: self.get_current_text_area().event_generate("<<Copy>>"))
        edit_menu.add_command(label="Paste", command=self.paste_text)
        menubar.add_cascade(label="Edit", menu=edit_menu)

        # Settings menu
        settings_menu = tk.Menu(menubar, tearoff=0)
        settings_menu.add_command(label="Color Scheme", command=lambda: set_color_scheme(
            self.root, self.get_current_text_area()))
    ...
    def create_new_sub_tab(self, sub_tab_control):
        ...
        prompt_label = tk.Label(input_frame, text="Input:",
                                bg=self.primary_color, fg=get_contrasting_color(self.primary_color))
        ...
        submit_button = tk.Button(input_frame, text="Submit", command=self.submit_input,
                                  width=10, bg=self.off_color, fg=get_contrasting_color(self.off_color))
        ...
        user_output_label = tk.Label(output_frame, text="User Output:",
                                     bg=self.primary_color, fg=get_contrasting_color(self.primary_color))
        user_output_label.pack()

        user_output_text = tk.Text(output_frame, wrap=tk.WORD, width=80,
                                   height=10, bg=self.off_color, fg=get_contrasting_color(self.off_color))
        user_output_text.pack(pady=5)

        user_scrollbar = tk.Scrollbar(
            output_frame, command=user_output_text.yview)
        ...
        diagnostic_output_label = tk.Label(
            diagnostic_frame, text="Diagnostic Output:", bg=self.primary_color, fg=get_contrasting_color(self.primary_color))
        diagnostic_output_label.pack()

        diagnostic_output_text = tk.Text(
            diagnostic_frame, wrap=tk.WORD, width=80, height=10, bg=self.off_color, fg=get_contrasting_color(self.off_color))
        diagnostic_output_text.pack(pady=5)

        diagnostic_scrollbar = tk.Scrollbar(
            diagnostic_frame, command=diagnostic_output_text.yview)
        diagnostic_output_text.configure(
            yscrollcommand=diagnostic_scrollbar.set)
    ...
    def load_program(self):
        ...
        self.get_current_diagnostic_area().insert(
            tk.END, f"Loading program from {file_path}\n")
        if file_path:
            ...
            self.get_current_diagnostic_area().insert(
                tk.END, f"Stored file path for tab: {file_path}\n")
    ...
    def save_program_as(self):
        ...
        if file_path:
            ...
            self.get_current_sub_tab_control().tab(
                current_tab, text=file_path.split('/')[-1])
        else:
    ...
    def run_program(self):
        ...
        program_code = self.get_current_text_area().get(
    ...
    def rerun_program(self):
        ...
        file_path = self.tab_files[current_tab]['file_path']
        self.get_current_diagnostic_area().insert(
            tk.END, f"Attempting to rerun program from {file_path}\n")
        if file_path:
            self.clear_outputs()  # Clear outputs before running the program
            self.simulator.load_program_from_file(file_path)
            self.simulator.run()
    ...
    def paste_text(self):
        ...
        current_content = self.get_current_text_area().get(
            "1.0", tk.END).strip().splitlines()
    ...
    def submit_input(self, event=None):
        if self.prompt_active:
            ...
            self.tab_controls[current_tab]['prompt_label'].config(
                text="Input:")  # Reset the prompt label
            self.prompt_active = False
